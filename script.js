...
function showUserMap(lat, lon) {
  const mapContainer = document.getElementById("userMapGoogle"); // Changed to prevent conflict with Leaflet in index.html
...
// Set your Gemini API key here for full conversational AI!
// Get a free key at: https://aistudio.google.com/app/apikey
const GEMINI_API_KEY = "AIzaSyCHGfeFxXgkepY_xnEXs1UzTTP0tvh2SCc"; 
...
function initAssistant() {
  ...
  if (!SpeechRecognition) {
    ...
    recognition.addEventListener('start', () => {
      ...
      micBtn.style.background = 'rgba(239, 68, 68, 0.5)'; // Glow red to show recording
    });
  ...
  function closeAssistant() {
    ...
    setTimeout(() => { panel.hidden = true; }, 300); // wait for fade out
...
function addChatMessage(sender, text) {
  ...
  msgDiv.className = `p-2 rounded-lg max-w-[85%] text-sm ${sender === 'user' ? 'bg-blue-500/20 self-end ml-auto text-right border border-blue-500/30 text-slate-900 dark:text-white' : 'bg-white/40 dark:bg-slate-800/60 self-start mr-auto border border-white/20 text-slate-800 dark:text-slate-200'}`;
...
async function handleChatSubmit(text) {
  ...
  // 1. Overrides: Fast App Navigation Commands
  if (t.includes('ambulance') || t.includes('108') || t.includes('call ambulance')) {
    ...
    if (confirm('Call ambulance (108) now?')) { callNumber('108'); reply = 'Calling ambulance now.'; }
    ...
    reply = 'Opening blood banks list.'; navigateTo('blood.html');
  } else if (t.includes('hospital') || t.includes('hospitals')) {
    reply = 'Opening nearby hospitals.'; navigateTo('hospital.html');
  }

  // 2. If no command matched, fetch answer from AI
  if (!reply) {
    const history = document.getElementById('chatHistory');
    ...
    if (history) {
      ...
      loadingDiv.className = 'p-2 rounded-lg max-w-[85%] text-sm bg-white/40 dark:bg-slate-800/60 self-start mr-auto border border-white/20 text-slate-500 italic';
...
async function askAI(query) {
  ...
  // Fallback to Wikipedia API (Free, no API key needed!)
  try {
  ...
  return "I'm sorry, I don't have an answer for that right now. (Developer: Please set GEMINI_API_KEY at the top of script.js to enable advanced AI answers!)";
}
